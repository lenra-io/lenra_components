...
void main() {
  ...
  testWidgets('LenraColumn test main centered position',
  ...
  testWidgets('LenraColumn test main spaceBetween position',
      (WidgetTester tester) async {
    await tester.pumpWidget(
      createBaseTestWidgets(
        MaterialApp(
    ...
    expect(tester.getTopLeft(find.byType(TextButton)).dy,
        equals(600 - tester.getSize(find.byType(TextButton)).height));
    expect(tester.getTopLeft(find.byType(LenraColumn)).dy, equals(0.0));
  });
  ...
  testWidgets('LenraColumn test spacing position', (WidgetTester tester) async {
    ...
    expect(tester.getSize(find.byType(SizedBox)).height,
        equals(LenraThemeData().baseSize * 10));
    expect(tester.getTopLeft(find.byKey(Key("A"))).dy, equals(0.0));
    expect(tester.getTopLeft(find.byKey(Key("B"))).dy,
        equals(30 + LenraThemeData().baseSize * 10));
  });

  testWidgets('LenraColumn test main SpaceEvenly position',
      (WidgetTester tester) async {
    await tester.pumpWidget(
      createBaseTestWidgets(
        MaterialApp(
  ...
  testWidgets('LenraRow test main SpaceAround position',
    ...
    expect(
        tester.getTopLeft(find.byKey(Key("A"))).dy.toInt(), equals(x.toInt()));
    expect(tester.getTopLeft(find.byKey(Key("B"))).dy.toInt(),
        equals((3 * x + 30).toInt()));
    expect(tester.getTopLeft(find.byKey(Key("C"))).dy.toInt(),
